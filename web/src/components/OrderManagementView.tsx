...
import type { ShipperResponse } from '@/types/api';
...
export default function OrderManagementView() {
    ...
    const fetchUsers = async () => {
        try {
            const data = await httpClient.get<{ username: string; userRole: string }[]>('/users');
            setUsers(data || []);
        } catch {
    ...
    const createOrder = async (orderData: OrderFormData) => {
        try {
            ...
            notify.error(error instanceof Error ? error.message : 'Failed to create order');
        }
    };
    ...
    const assignPicker = async () => {
        if (!assignPickerData.assignedTo) {
            notify.error('Please select a picker');
            return;
        }
        try {
            await httpClient.post('/orders/assign-pickers', assignPickerData);
            notify.success('Picker assigned successfully');
            setShowAssignModal(false);
            fetchOrders();
            setAssignPickerData({ orderId: '', assignedTo: '' });
        } catch (error: unknown) {
            notify.error(error instanceof Error ? error.message : 'Failed to assign picker');
        }
    };

    const updateOrderStatus = async (orderId: string, action: string) => {
        try {
            await httpClient.put(`/orders/${orderId}/${action}`, {});
            notify.success(`Order ${action.replace('mark-', '')} successfully`);
            fetchOrders();
        } catch {
            notify.error(`Failed to ${action}`);
        }
    };
    ...
    const handleDispatch = async () => {
        if (!dispatchData.warehouseId) {
            notify.error('Please select an origin warehouse');
            return;
        }
        ...
        try {
            ...
            notify.success('Shipment created and order dispatched!');
            setShowDispatchModal(false);
        } catch (error) {
            console.error(error);
            notify.error('Failed to create shipment or dispatch order');
        }
    };
    ...
    const cancelOrder = async (orderId: string) => {
        ...
        try {
            await httpClient.put(`/orders/${orderId}/cancel`, {});
    ...
    if (loading && orders.length === 0) {
        return <LoadingSpinner message="Synchronizing Order Pipeline..." />;
    }
