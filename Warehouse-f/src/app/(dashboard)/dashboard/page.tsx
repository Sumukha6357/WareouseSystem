...
export default function DashboardPage() {
    ...
    const fetchPickerStats = async () => {
        ...
        try {
            ...
            if (response.data.status === 200) {
                ...
                const active = tasks.filter((t: any) => t.status === 'ASSIGNED' || t.status === 'IN_PROGRESS').length;
    ...
    const fetchManagerStats = async () => {
        try {
            ...
            const activeShipments = shipments.filter((s: any) => ['CREATED', 'PICKED', 'PACKED', 'IN_TRANSIT'].includes(s.status)).length;
    ...
    const fetchShipperStats = async () => {
        ...
        try {
            ...
            const myShipments = allShipments.filter((s: any) => s.shipperName === user.username);

            const active = myShipments.filter((s: any) => s.status === 'IN_TRANSIT').length;
            const pending = myShipments.filter((s: any) => ['CREATED', 'PICKED', 'PACKED', 'WAITING_FOR_DRIVER'].includes(s.status)).length;
            const completed = myShipments.filter((s: any) => s.status === 'DELIVERED').length;
    ...
    const fetchPackingStats = async () => {
        try {
            ...
            const readyToPack = allShipments.filter((s: any) => s.status === 'PICKED').length;
            // Packed = status is 'PACKED'
            const packed = allShipments.filter((s: any) => s.status === 'PACKED').length;
            // Total eligible for packer interaction (roughly)
            const total = allShipments.filter((s: any) => ['PICKED', 'PACKED'].includes(s.status)).length;
    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            if (user.userRole === 'PICKER') {
                ...
                fetchStats(); // Managers still need product/inventory stats
                fetchManagerStats();
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="relative h-48 rounded-3xl overflow-hidden group">
                                    <img
                                        src="https://images.unsplash.com/photo-1586528116311-ad86d304a919?auto=format&fit=crop&q=80&w=800"
            ...
            >
                <div className="h-24 flex items-center px-8 border-b border-card-border bg-sidebar/50 backdrop-blur-sm">
                    <div className="h-12 w-12 bg-gradient-to-br from-primary to-emerald-800 rounded-2xl flex items-center justify-center shadow-lg shadow-primary/20 animate-float overflow-hidden">
                        <img src="/warehouse_logo.svg" alt="WarehouseMS" className="h-8 w-8 object-contain" />
                    </div>
                    {isSidebarOpen && (
                        ...
                        <div className="flex items-center gap-8">
                            ...
                            <div className="relative" ref={profileRef}>
                                ...
                                >
                                    ...
                                    <div className="h-12 w-12 bg-gradient-to-br from-gray-800 to-black dark:from-gray-700 dark:to-gray-900 rounded-2xl flex items-center justify-center text-white font-black shadow-xl shadow-black/10 dark:shadow-black/40 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 overflow-hidden">
                                        {user.profileImage ? (
                                ...
                                {isProfileOpen && (
                                    <div className="absolute right-0 mt-4 w-72 bg-card rounded-3xl shadow-2xl border border-card-border py-3 z-50 animate-in fade-in slide-in-from-top-4 duration-300">
                                        <div className="px-6 py-4 border-b border-card-border bg-background/50 rounded-t-3xl">
                                            <div className="flex items-center gap-3">
                                                <div className="h-10 w-10 bg-primary rounded-xl flex items-center justify-center text-white font-black overflow-hidden">
                                                    {user.profileImage ? (
                                                        <img src={user.profileImage} alt={user.username} className="h-full w-full object-cover" />
                                                    ) : (
                                                        user.username.charAt(0).toUpperCase()
