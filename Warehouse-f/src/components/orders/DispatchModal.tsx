...
export default function DispatchModal({
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-6 animate-in fade-in duration-300">
            <div className="bg-card rounded-[3.5rem] p-12 max-w-xl w-full border-2 border-card-border shadow-2xl animate-in zoom-in-95">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-3xl font-black text-sharp tracking-tighter">Exit Logistics</h2>
                        <p className="text-sm font-medium text-muted mt-1">Initiate outward transport protocol</p>
                    </div>
                    <div className="p-4 bg-emerald-500/5 rounded-3xl text-emerald-600 border border-emerald-500/10">
                        <Truck size={32} />
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3">
                    <Button
                        onClick={onDispatch}
                        className="w-full py-6 bg-emerald-600 hover:bg-emerald-700 text-xs font-black uppercase tracking-widest"
                        isLoading={isLoading}
                    >
                        Execute Dispatch
                    </Button>
