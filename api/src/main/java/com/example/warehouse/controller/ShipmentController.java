...
public class ShipmentController {
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN','STAFF','WAREHOUSE_MANAGER','SUPERVISOR','PACKER','EXTERNAL_SHIPPER')")
    public ResponseEntity<ResponseStructure<PageResponse<ShipmentResponse>>> getAllShipmentsPaged(Pageable pageable) {
        List<ShipmentResponse> shipments = shipmentService.getAllShipments().stream().map(this::toResponse).toList();
        PageResponse<ShipmentResponse> pageResponse = PageUtils.toPageResponse(PageUtils.paginate(shipments, pageable));
        return ResponseEntity.ok(new ResponseStructure<>(HttpStatus.OK.value(), "All shipments retrieved successfully", pageResponse));
    }
    ...
    public ResponseEntity<ResponseStructure<ShipmentResponse>> createShipment(@Valid @RequestBody CreateShipmentRequest request) {
        Shipment shipment = shipmentService.createShipment(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(new ResponseStructure<>(HttpStatus.CREATED.value(), "Shipment created successfully",
                        toResponse(shipment)));
    }
    ...
    @ApiResponse(responseCode = "200", description = "Shipment found", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
            ...
            """)))
    public ResponseEntity<ResponseStructure<ShipmentResponse>> getShipmentById(@PathVariable @NotBlank String shipmentId) {
        Shipment shipment = shipmentService.getShipmentById(shipmentId);
        return ResponseEntity.ok(new ResponseStructure<>(HttpStatus.OK.value(), "Shipment found", toResponse(shipment)));
    }

    @GetMapping("/order/{orderId}")
    @PreAuthorize("hasAnyAuthority('ADMIN','STAFF','WAREHOUSE_MANAGER','SUPERVISOR','PACKER','EXTERNAL_SHIPPER')")
    public ResponseEntity<ResponseStructure<List<ShipmentResponse>>> getShipmentsByOrder(@PathVariable @NotBlank String orderId) {
    ...
    public ResponseEntity<ResponseStructure<PageResponse<ShipmentResponse>>> getShipmentsByOrderPaged(
        ...
        List<ShipmentResponse> shipments = shipmentService.getShipmentsByOrderId(orderId).stream().map(this::toResponse).toList();
        PageResponse<ShipmentResponse> pageResponse = PageUtils.toPageResponse(PageUtils.paginate(shipments, pageable));
        return ResponseEntity.ok(new ResponseStructure<>(HttpStatus.OK.value(), "Shipments retrieved successfully", pageResponse));
    }

    @GetMapping("/status/{status}")
    @PreAuthorize("hasAnyAuthority('ADMIN','STAFF','WAREHOUSE_MANAGER','SUPERVISOR','PACKER','EXTERNAL_SHIPPER')")
    public ResponseEntity<ResponseStructure<List<ShipmentResponse>>> getShipmentsByStatus(@PathVariable ShipmentStatus status) {
    ...
    public ResponseEntity<ResponseStructure<PageResponse<ShipmentResponse>>> getShipmentsByStatusPaged(
        ...
        List<ShipmentResponse> shipments = shipmentService.getShipmentsByStatus(status).stream().map(this::toResponse).toList();
        PageResponse<ShipmentResponse> pageResponse = PageUtils.toPageResponse(PageUtils.paginate(shipments, pageable));
        return ResponseEntity.ok(new ResponseStructure<>(HttpStatus.OK.value(), "Shipments retrieved successfully", pageResponse));
    }
    ...
    public ResponseEntity<ResponseStructure<List<ShipmentResponse>>> getActiveShipments() {
        ...
        return ResponseEntity.ok(
                new ResponseStructure<>(HttpStatus.OK.value(), "Active shipments retrieved successfully",
                        shipments.stream().map(this::toResponse).toList()));
    }

    @GetMapping(value = "/active", params = {"page", "size"})
    @PreAuthorize("hasAnyAuthority('ADMIN','STAFF','WAREHOUSE_MANAGER','SUPERVISOR','PACKER','EXTERNAL_SHIPPER')")
    public ResponseEntity<ResponseStructure<PageResponse<ShipmentResponse>>> getActiveShipmentsPaged(Pageable pageable) {
        List<ShipmentResponse> shipments = shipmentService.getActiveShipments().stream().map(this::toResponse).toList();
        PageResponse<ShipmentResponse> pageResponse = PageUtils.toPageResponse(PageUtils.paginate(shipments, pageable));
        return ResponseEntity.ok(new ResponseStructure<>(HttpStatus.OK.value(), "Active shipments retrieved successfully", pageResponse));
    }
